...
I struggled for months to understand even the

easiest concepts.

In some sense, this is entirely normal.
...
and "quantum mysteries".

Instead, a large part of this book is
...
I wrote the book I wished had existed when
...
is by no means an authoritative reference.
...
cratic hand-drawn illustrations.

Usually, textbooks include
...
book is assumed to be "obvious" or "easy to see".
...
We start with a "Bird’s Eye View" of quantum mechanics
...
We will stick to the most fundamental examples that
...
nian formulations of classical mechanics

learning these alterna- formulation.

function formulation and ignore the alternatives.
...
> The most important equations and results are highlighted

like this:

> We often use the shorthand notation ∂_i = 2 for the partial
...
> The absolute value of a quantity p is denoted by |p|.

That's it.

We are ready to dive in (after a short look at the table
...
Our goal is not to understand everything
...
allow only a discrete set of values.

We say they are quantized.
...
these features of quantum systems in mathematical terms.

first essential ingredient is a new type of objects

called "kets".

We use kets to describe the system in question.
...
” The explicit form of the quantum operators
...
tity: angular momentum.

In quantum mechanics, there are two
...
It is not necessary to study it in detail at
...
We will talk about all fundamentally im-

portant concepts in just a few pages.
...
eigenstate of the operator in question.

For each operator, there
...
by a star *, and matrix transposition
...
we will not always get the same result.

Instead, we sometimes

(with probability |a_i|²) measure the value p_i and sometimes

other values like p_2.

To make sense of a system which is not in an eigenstate
...
instead the bra ⟨ψ| corresponding to the ket |ψ⟩.
...
Formulated differently, calculating the
...
Another important observation is that for each operator
...
where |o⟩ are the eigenstates corresponding to a different oper-
...
yields coefficients (a_i or b_i) which are directly related to the
...
Take note that our discrete coefficients a_i are replaced by a func-
...
The probability of measuring the value o is
...
Analogous to how we can describe a given vector v using the
...
experiment "has in it the heart of quantum mechanics.
...
This experimental setup is known as the double slit
...
This pattern arises because the wave goes through both
...
and all other elementary particles are particles.
...
Our detectors make "click, click,
...
completely without using waves at all.
...
where is an integer and is the absolute value of the charge
...
or , but never something like or .
...
It was also not immediately discovered when physicists

were developing electrodynamics because the charges of macro-

scopic objects (e.g.

a metal sphere) are typically on the order of

Compared to the charge of a single electron
...
We can observe this by measuring the radiation emitted

whenever an electron jumps from one energy level to a lower

one.

The energy of the radiation is precisely the energy differ-
...
This is where the notion of quantum jumps
...
No matter how the two hands try to make the rope vibrate
...
directly related to its energy as¹³ ¹³ We will discuss the origin of this

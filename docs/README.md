...
- 当前收录：**417 本**（中文 171 / 英文 246）
- 零运行时依赖：只有 HTML + CSS + 原生 JS
- 文本版 PDF 直接提取，扫描版 PDF 用 Tesseract + LLM 校对识别
- 阅读体验：每本书自动生成**章节目录**（可跳转）、**夜间模式**、**字号 / 版宽调节**（本地记忆）
- 检索与浏览：**全文检索**（搜书名 / 作者 / 正文片段 / 章节）、**主题分类筛选**（物理 / 数学 / 艺术 / 历史 …）、中英文筛选、按书名 / 作者排序
- 数据清洗：`data_fix.py` 去垃圾条目、修损坏书名、补主题与作者；`ocr_local_fix.py` 本地修复 OCR 文本（去伪标题、折叠中文间空格、合并断行）
...
└── Z-Library-Web/                ← 本项目（GitHub 就绪）
    ...
    │   ├── build_site.py        把 books/ 渲染成 site/ 静态站（含目录 / 检索 / 主题）
    │   ├── data_fix.py          书目清洗：去垃圾、修标题、补主题与作者
    │   ├── ocr_local_fix.py     本地 OCR 文本修复（无 API）
...
`site/index.html` 支持：**全文检索**（书名 / 作者 / 正文 / 章节）、**主题分类筛选**、中英文筛选、按书名 / 作者排序。
单本阅读页支持：左侧**章节目录**跳转、**夜间模式**、**字号与版宽调节**（设置记忆在浏览器本地）。
